...
public class CharSequenceInputStream extends InputStream
{
	 ...
	 */
	@Override
	public synchronized void mark(@SuppressWarnings("unused")
	final int readlimit)
	{
	...
	public int read() throws IOException
	{
		for (;;)
		{
			...
			{
				this.fillBuffer();
				if (!this.bbuf.hasRemaining() && !this.cbuf.hasRemaining()) { return -1; }
			}
		}
	}
	...
	public int read(final byte[] b, int off, int len) throws IOException
	{
		...
		if (len == 0) { return 0; // must return 0 for zero length read
		}
		if (!this.bbuf.hasRemaining() && !this.cbuf.hasRemaining()) { return -1; }
		int bytesRead = 0;
		while (len > 0)
