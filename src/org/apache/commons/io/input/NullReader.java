...
public class NullReader extends Reader
{
	 ...
	 */
	public NullReader(final long size, final boolean markSupported,
	...
	{
		if (!this.markSupported) { throw new UnsupportedOperationException(
				"Mark not supported"); }
	...
	{
		if (this.eof) { throw new IOException("Read after end of file"); }
		if (this.position == this.size) { return this.doEndOfFile(); }
		this.position++;
	...
	public synchronized void reset() throws IOException
	{
		if (!this.markSupported) { throw new UnsupportedOperationException(
				"Mark not supported"); }
		if (this.mark < 0) { throw new IOException(
				"No position has been marked"); }
		if (this.position > (this.mark + this.readlimit)) { throw new IOException(
				"Marked position [" + this.mark +
	...
	{
		if (this.eof) { throw new IOException("Skip after end of file"); }
		if (this.position == this.size) { return this.doEndOfFile(); }
	...
	{
		this.eof = true;
		if (this.throwEofException) { throw new EOFException(); }
		return -1;
	}
