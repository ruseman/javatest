...
public class ReversedLinesFileReader implements Closeable
{
	...
	private class FilePart
	{
		 ...
		 */
		private FilePart(final long no, final int length,
				final byte[] leftOverOfLastFilePart) throws IOException
		{
			this.no = no;
			final int dataLength = length
					+ (leftOverOfLastFilePart != null ? leftOverOfLastFilePart.length
							: 0);
			...
			if (no > 0 /* file not empty */)
			{
				...
				final int countRead = ReversedLinesFileReader.this.randomAccessFile
						.read(this.data, 0, length);
				if (countRead != length) { throw new IllegalStateException(
						"Count of requested bytes and actually read bytes don't match"); }
			}
			// copy left over part into data arr
			if (leftOverOfLastFilePart != null)
			{
				System.arraycopy(leftOverOfLastFilePart, 0, this.data, length,
		...
		private void createLeftOver()
		{
			...
			if (lineLengthBytes > 0)
			{
				...
				System.arraycopy(this.data, 0, this.leftOver, 0,
						lineLengthBytes);
			}
			else
			{
		...
		private int getNewLineMatchByteCount(final byte[] data, final int i)
		{
			for (final byte[] newLineSequence : ReversedLinesFileReader.this.newLineSequences)
			{
				...
				if (match) { return newLineSequence.length; }
			}
			return 0;
		}
		...
		private String readLine() throws IOException
		{
			...
			while (i > -1)
			{
				...
				if (!isLastFilePart
						&& (i < ReversedLinesFileReader.this.avoidNewlineSplitBufferSize))
				{
					// avoidNewlineSplitBuffer: for all except the last file
// part we
					// take a few bytes to the next file part to avoid splitting
// of newlines
					this.createLeftOver();
					break; // skip last few bytes and leave it to the next file
// part
				}
				
				// --- check for newline ---
				if ((newLineMatchByteCount = this.getNewLineMatchByteCount(
						this.data, i)) > 0 /* found newline */)
				{
					final int lineStart = i + 1;
					final int lineLengthBytes = (this.currentLastBytePos - lineStart) + 1;
					
					if (lineLengthBytes < 0) { throw new IllegalStateException(
							"Unexpected negative line length="
									+ lineLengthBytes); }
					final byte[] lineData = new byte[lineLengthBytes];
					System.arraycopy(this.data, lineStart, lineData, 0,
							lineLengthBytes);
					
					line = new String(lineData,
							ReversedLinesFileReader.this.encoding);
...
// of the file
		...
		private FilePart rollOver() throws IOException
		{
			...
			if (this.currentLastBytePos > -1) { throw new IllegalStateException(
			...
			if (this.no > 1)
			{
				return new FilePart(this.no - 1,
						ReversedLinesFileReader.this.blockSize, this.leftOver);
			}
			else
			{
				// NO 1 was the last FilePart, we're finished
				if (this.leftOver != null) { throw new IllegalStateException(
						"Unexpected leftover of the last block: leftOverOfThisFilePart="
								+ new String(this.leftOver,
										ReversedLinesFileReader.this.encoding)); }
				return null;
			}
		}
	 ...
	 * @since 2.3
	 */
	public ReversedLinesFileReader(final File file, final int blockSize,
			final Charset encoding) throws IOException
	{
		...
		this.currentFilePart = new FilePart(this.totalBlockCount,
				lastBlockLength, null);
		...
		{
			// UTF-8 works fine out of the box, for multibyte sequences a second
// UTF-8 byte can never be a newline byte
		...
		else if ((charset == Charset.forName("UTF-16BE"))
				|| (charset == Charset.forName("UTF-16LE")))
		{
			// UTF-16 new line sequences are not allowed as second tuple of four
// byte sequences,
...
// important that \r\n is BEFORE \n
	...
	public String readLine() throws IOException
	{
		...
		// aligned behaviour wiht BufferedReader that doesn't return a last,
// emtpy line
