 ...
 * and return the subsequent byte as the
...
public class BOMInputStream extends ProxyInputStream
{
	...
	private static final Comparator<ByteOrderMark>	ByteOrderMarkLengthComparator	= new Comparator<ByteOrderMark>()
																					{
																						...
																						@Override
																						public int compare(
																								final ByteOrderMark bom1,
																								final ByteOrderMark bom2)
																						{
																							final int len1 = bom1
																									.length();
																							final int len2 = bom2
	...
	{
		super(delegate);
		if ((boms == null) || (boms.length == 0)) { throw new IllegalArgumentException(
				"No BOMs specified"); }
...
// the same starting two bytes.
	...
	{
		if (!this.boms.contains(bom)) { throw new IllegalArgumentException(
	...
	private ByteOrderMark find()
	{
		...
		{
			if (this.matches(bom)) { return bom; }
		}
		return null;
	}
	...
	private boolean matches(final ByteOrderMark bom)
	{
		...
		{
			if (bom.get(i) != this.firstBytes[i]) { return false; }
		}
		return true;
	}
