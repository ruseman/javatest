...
public class CharSequenceReader extends Reader implements Serializable
{
	...
	{
		if (this.idx >= this.charSequence.length()) { return -1; }
		if (array == null) { throw new NullPointerException(
				"Character array is missing"); }
		if ((length < 0) || (offset < 0) || ((offset + length) > array.length)) { throw new IndexOutOfBoundsException(
	...
	public long skip(final long n)
	{
		...
		if (this.idx >= this.charSequence.length()) { return -1; }
		final int dest = (int) Math.min(this.charSequence.length(), this.idx
				+ n);
