...
public class XmlStreamReader extends Reader
{
	...
	private static final ByteOrderMark[]	XML_GUESS_BYTES		= new ByteOrderMark[]
																{
			new ByteOrderMark(XmlStreamReader.UTF_8, 0x3C, 0x3F, 0x78, 0x6D),
			new ByteOrderMark(XmlStreamReader.UTF_16BE, 0x00, 0x3C, 0x00, 0x3F),
			new ByteOrderMark(XmlStreamReader.UTF_16LE, 0x3C, 0x00, 0x3F, 0x00),
			new ByteOrderMark(XmlStreamReader.UTF_32BE, 0x00, 0x00, 0x00, 0x3C,
					0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00,
					0x00, 0x6D),
			new ByteOrderMark(XmlStreamReader.UTF_32LE, 0x3C, 0x00, 0x00, 0x00,
					0x3F, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x6D, 0x00,
					0x00, 0x00),
			new ByteOrderMark(XmlStreamReader.EBCDIC, 0x4C, 0x6F, 0xA7, 0x94)
																};
	...
	static String getContentTypeEncoding(final String httpContentType)
	{
		...
		if (httpContentType != null)
		{
			...
			if (i > -1)
			{
				...
				final Matcher m = XmlStreamReader.CHARSET_PATTERN
						.matcher(postMime);
	 ...
	 */
	private static String getXmlProlog(final InputStream is,
			final String guessedEnc)
			throws IOException
	{
		String encoding = null;
		if (guessedEnc != null)
		{
			...
			while ((c != -1) && (firstGT == -1)
					&& (offset < XmlStreamReader.BUFFER_SIZE))
			{
				offset += c;
				max -= c;
			...
			if (bytesRead > 0)
			{
				...
				final Matcher m = XmlStreamReader.ENCODING_PATTERN
						.matcher(prolog);
				if (m.find())
				{
			...
			final String defaultEncoding) throws IOException
	{
		...
		final BOMInputStream bom = new BOMInputStream(new BufferedInputStream(
				is, XmlStreamReader.BUFFER_SIZE), false, XmlStreamReader.BOMS);
	 ...
	 */
	public XmlStreamReader(final InputStream is, final String httpContentType,
			final boolean lenient, final String defaultEncoding)
			throws IOException
	{
		this.defaultEncoding = defaultEncoding;
		final BOMInputStream bom = new BOMInputStream(new BufferedInputStream(
	 ...
	 *             the URLConnection.
	 */
	public XmlStreamReader(final URLConnection conn,
			final String defaultEncoding) throws IOException
	{
		this.defaultEncoding = defaultEncoding;
			...
			final boolean lenient) throws IOException
	{
		...
		if (lenient && (xmlEnc != null)) { return xmlEnc; }
		
		// Determine mime/encoding content types from HTTP Content Type
		final String cTMime = XmlStreamReader
				.getContentTypeMime(httpContentType);
		final String cTEnc = XmlStreamReader
				.getContentTypeEncoding(httpContentType);
		...
		if (!appXml && !textXml)
		{
			final String msg = MessageFormat.format(XmlStreamReader.HTTP_EX_3,
					cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
			throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc,
					xmlGuessEnc, xmlEnc);
		}
		...
		if (cTEnc == null)
		{
			...
			{
				return this.defaultEncoding == null ? XmlStreamReader.US_ASCII
						: this.defaultEncoding;
			}
		}
		
		// UTF-16BE or UTF-16LE content type encoding
		if (cTEnc.equals(XmlStreamReader.UTF_16BE)
				|| cTEnc.equals(XmlStreamReader.UTF_16LE))
		{
			if (bomEnc != null)
			{
				final String msg = MessageFormat.format(
						XmlStreamReader.HTTP_EX_1, cTMime, cTEnc, bomEnc,
						xmlGuessEnc, xmlEnc);
				throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc,
						xmlGuessEnc, xmlEnc);
			}
		...
		{
			if ((bomEnc != null) && bomEnc.startsWith(XmlStreamReader.UTF_16)) { return bomEnc; }
			final String msg = MessageFormat.format(XmlStreamReader.HTTP_EX_2,
					cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
			throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc,
					xmlGuessEnc, xmlEnc);
		}
		...
		if (cTEnc.equals(XmlStreamReader.UTF_32BE)
				|| cTEnc.equals(XmlStreamReader.UTF_32LE))
		{
			if (bomEnc != null)
			{
				final String msg = MessageFormat.format(
		...
		{
			if ((bomEnc != null) && bomEnc.startsWith(XmlStreamReader.UTF_32)) { return bomEnc; }
			final String msg = MessageFormat.format(XmlStreamReader.HTTP_EX_2,
					cTMime, cTEnc, bomEnc, xmlGuessEnc, xmlEnc);
			throw new XmlStreamReaderException(msg, cTMime, cTEnc, bomEnc,
					xmlGuessEnc, xmlEnc);
			...
			final String xmlEnc) throws IOException
	{
		...
		if (bomEnc == null)
		{
			if ((xmlGuessEnc == null) || (xmlEnc == null)) { return this.defaultEncoding == null ? XmlStreamReader.UTF_8
					: this.defaultEncoding; }
			if (xmlEnc.equals(XmlStreamReader.UTF_16)
					&&
					(xmlGuessEnc.equals(XmlStreamReader.UTF_16BE) || xmlGuessEnc
							.equals(XmlStreamReader.UTF_16LE))) { return xmlGuessEnc; }
			return xmlEnc;
		}
		...
		{
			if ((xmlGuessEnc != null)
					&& !xmlGuessEnc.equals(XmlStreamReader.UTF_8))
			{
				final String msg = MessageFormat.format(
						XmlStreamReader.RAW_EX_1, new Object[]
						{
		...
		if (bomEnc.equals(XmlStreamReader.UTF_16BE)
				|| bomEnc.equals(XmlStreamReader.UTF_16LE))
		{
			...
			if ((xmlEnc != null) && !xmlEnc.equals(XmlStreamReader.UTF_16)
		...
		if (bomEnc.equals(XmlStreamReader.UTF_32BE)
				|| bomEnc.equals(XmlStreamReader.UTF_32LE))
		{
			if ((xmlGuessEnc != null) && !xmlGuessEnc.equals(bomEnc))
			{
				final String msg = MessageFormat.format(
			...
			if ((xmlEnc != null) && !xmlEnc.equals(XmlStreamReader.UTF_32)
					&& !xmlEnc.equals(bomEnc))
			{
				final String msg = MessageFormat.format(
						XmlStreamReader.RAW_EX_1, new Object[]
		...
		final String msg = MessageFormat.format(XmlStreamReader.RAW_EX_2,
				new Object[]
				{
						bomEnc, xmlGuessEnc, xmlEnc
				});
	 ...
	 *             thrown if there is a problem reading the stream.
	 */
	private String doHttpStream(final BOMInputStream bom,
			final BOMInputStream pis, final String httpContentType,
			final boolean lenient) throws IOException
	{
			...
			XmlStreamReaderException ex) throws IOException
	{
				...
				&& httpContentType.startsWith("text/html"))
		{
			...
			try
			{
				return this.calculateHttpEncoding(httpContentType,
						ex.getBomEncoding(),
		...
		if (encoding == null)
		{
			encoding = this.defaultEncoding == null ? XmlStreamReader.UTF_8
					: this.defaultEncoding;
		}
		return encoding;
	}
	 ...
	 *             thrown if there is a problem reading the stream.
	 */
	private String doRawStream(final BOMInputStream bom,
			final BOMInputStream pis, final boolean lenient)
			throws IOException
	{
		final String bomEnc = bom.getBOMCharsetName();
		final String xmlGuessEnc = pis.getBOMCharsetName();
