...
public class ReaderInputStream extends InputStream
{
	...
	public int read() throws IOException
	{
		for (;;)
		{
			...
			{
				this.fillBuffer();
				if (this.endOfInput && !this.encoderOut.hasRemaining()) { return -1; }
			}
		}
	}
	...
	public int read(final byte[] b, int off, int len) throws IOException
	{
		...
		if ((len < 0) || (off < 0) || ((off + len) > b.length)) { throw new IndexOutOfBoundsException(
				"Array Size=" + b.length +
	...
	{
		if (!this.endOfInput
				&& ((this.lastCoderResult == null) || this.lastCoderResult
						.isUnderflow()))
		{
...
// implementation
			...
			final int c = this.reader.read(this.encoderIn.array(), position,
					this.encoderIn.remaining());
			if (c == -1)
			{
		...
		this.lastCoderResult = this.encoder.encode(this.encoderIn,
				this.encoderOut, this.endOfInput);
		this.encoderOut.flip();
	}
}
