		...
		Serializable
{
	...
	public static final Comparator<File>	SIZE_REVERSE			= new ReverseComparator(
	 ...
	 */
	public static final Comparator<File>	SIZE_SUMDIR_COMPARATOR	= new SizeFileComparator(
																			true);
	 ...
	 * using {@link FileUtils#sizeOfDirectory(File)}
	 */
	public static final Comparator<File>	SIZE_SUMDIR_REVERSE		= new ReverseComparator(
	 ...
	 *            {@code true} if the sum of the directoryies contents
	 *            should be calculated, otherwise {@code false} if directories
	 *            should be treated
	...
	public int compare(final File file1, final File file2)
	{
		...
		{
			size1 = this.sumDirectoryContents && file1.exists() ? FileUtils
					.sizeOfDirectory(file1) : 0;
		}
		else
		{
			size1 = file1.length();
		...
		{
			size2 = this.sumDirectoryContents && file2.exists() ? FileUtils
					.sizeOfDirectory(file2) : 0;
		}
		else
		{
			size2 = file2.length();
	...
	{
		return super.toString() + "[sumDirectoryContents="
				+ this.sumDirectoryContents + "]";
	}
}
