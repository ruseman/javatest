...
public class IOUtils
{
	 ...
	 */
	public static long copyLarge(final InputStream input,
			final OutputStream output, final long inputOffset, final long length)
			throws IOException
	{
			...
			throws IOException
	{
		...
		long totalRead = 0;
		while ((bytesToRead > 0)
				&& (IOUtils.EOF != (read = input.read(buffer, 0, bytesToRead))))
		{
	...
	{
		return IOUtils.copyLarge(input, output,
				new char[IOUtils.DEFAULT_BUFFER_SIZE]);
	}
	 ...
	 */
	public static long copyLarge(final Reader input, final Writer output,
			final long inputOffset, final long length, final char[] buffer)
			throws IOException
	{
		if (inputOffset > 0)
		{
			...
			final int offset, final int length) throws IOException
	{
		...
		if (actual != length) { throw new EOFException("Length to read: "
			...
			throws IOException
	{
		...
		while (remain > 0)
		{
			final long n = input.read(IOUtils.SKIP_BYTE_BUFFER, 0,
					(int) Math.min(remain, IOUtils.SKIP_BUFFER_SIZE));
			if (n < 0)
			{ // EOF
				break;
			}
			...
			throws IOException
	{
		...
		while (remain > 0)
		{
			final long n = input.read(IOUtils.SKIP_CHAR_BUFFER, 0,
					(int) Math.min(remain, IOUtils.SKIP_BUFFER_SIZE));
			if (n < 0)
			{ // EOF
				break;
	...
	{
		final long skipped = IOUtils.skip(input, toSkip);
		if (skipped != toSkip) { throw new EOFException("Chars to skip: "
				+ toSkip + " actual: " + skipped); }
	}
	 ...
	 * creates a toBufferedReader for the given
	 * reader.
	 * 
	 * @param reader
			...
			throws IOException
	{
		...
		while ((offset < size)
				&& ((readed = input.read(data, offset, size - offset)) != IOUtils.EOF))
		{
			offset += readed;
		}
	 ...
	 * before using {@link IOUtils#toByteArray(java.io.InputStream, int)} to
	...
	public static InputStream toInputStream(final String input,
			final Charset encoding)
	{
		return new ByteArrayInputStream(input.getBytes(Charsets
				.toCharset(encoding)));
	}
