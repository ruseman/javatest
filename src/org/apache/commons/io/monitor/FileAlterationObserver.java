...
public class FileAlterationObserver implements Serializable
{
	 ...
	 */
	public FileAlterationObserver(final File directory,
			final FileFilter fileFilter, final IOCase caseSensitivity)
	{
	 ...
	 *            system sensitive
	 */
	protected FileAlterationObserver(final FileEntry rootEntry,
			final FileFilter fileFilter, final IOCase caseSensitivity)
	{
		if (rootEntry == null) { throw new IllegalArgumentException(
				"Root entry is missing"); }
		if (rootEntry.getFile() == null) { throw new IllegalArgumentException(
	...
	public void checkAndNotify()
	{
		...
		if (rootFile.exists())
		{
			this.checkAndNotify(this.rootEntry, this.rootEntry.getChildren(),
					this.listFiles(rootFile));
		}
	...
	public void initialize() throws Exception
	{
		...
		final FileEntry[] children = files.length > 0 ? new FileEntry[files.length]
	 ...
	 */
	private void checkAndNotify(final FileEntry parent,
			final FileEntry[] previous, final File[] files)
	{
		int c = 0;
		final FileEntry[] current = files.length > 0 ? new FileEntry[files.length]
				: FileEntry.EMPTY_ENTRIES;
		for (final FileEntry entry : previous)
		{
			while ((c < files.length)
					&& (this.comparator.compare(entry.getFile(), files[c]) > 0))
			{
					...
					&& (this.comparator.compare(entry.getFile(), files[c]) == 0))
			{
				...
				this.checkAndNotify(entry, entry.getChildren(),
						this.listFiles(files[c]));
				current[c] = entry;
				c++;
			}
			else
	...
	private File[] listFiles(final File file)
	{
		...
		if (file.isDirectory())
		{
			children = this.fileFilter == null ? file.listFiles() : file
					.listFiles(this.fileFilter);
		}
		if (children == null)
		{
