 ...
 * </ul>
 * <a name="example"></a>
 * <h3>1. Example Implementation</h3> There are many possible extensions, for
 * example, to delete all files and '.svn' directories, and return a list of
 * deleted files:
 * 
 * <pre>
 ...
 *
 * <a name="filter"></a>
 * <h3>2. Filter Example</h3> Choosing which directories and files to process
 * can be a key aspect of using this class. This information can be setup in
 * three ways, via three different constructors.
 * <p>
 ...
 * preferred option for filtering. <a name="cancel"></a>
 * <h3>3. Cancellation</h3> The DirectoryWalker contains some of the logic
 * required for cancel processing. Subclasses must complete the implementation.
 * <p>
 ...
 * different scenarios. <a name="external"></a>
 * <h4>3.1 External / Multi-threaded</h4> This example provides a public
 * <code>cancel()</code> method that can be called by another thread to stop the
 * processing. A typical example use-case would be a cancel button on a GUI.
 * Calling this method sets a <a href=
 ...
 *
 * <a name="internal"></a>
 * <h4>3.2 Internal</h4> This shows an example of how internal cancellation
 * processing could be implemented. <b>Note</b> the decision logic and throwing
 * a {@link CancelException} could be implemented in any of the <i>lifecycle</i>
 * methods.
 *
 * <pre>
 ...
 * 		if (directory.isHidden()) { throw new CancelException(file, depth); }
 * 		return true;
 * 	}
 * 	
 * 	protected void handleFile(File file, int depth, Collection results)
 ...
 * 		if (!file.canWrite()) { throw new CancelException(file, depth); }
 * 		results.add(file);
 * 	}
 * 	
 * 	protected void handleCancelled(File startDirectory, Collection results,
...
public abstract class DirectoryWalker<T>
{
	...
	public static class CancelException extends IOException
	{
		 ...
		 */
		public CancelException(final String message, final File file,
				final int depth)
		{
			super(message);
			this.file = file;
	 ...
	 */
	protected final void checkIfCancelled(final File file, final int depth,
			final Collection<T> results) throws IOException
	{
		if (this.handleIsCancelled(file, depth, results)) { throw new CancelException(
				file, depth); }
	}
	 ...
	 * @since 2.0
	 */
	protected File[] filterDirectoryContents(final File directory,
			final int depth, final File[] files) throws IOException
	{
		return files;
	}
	 ...
	 *             if an I/O Error occurs
	 */
	protected void handleCancelled(final File startDirectory,
	...
	protected final void walk(final File startDirectory,
			final Collection<T> results) throws IOException
	{
		if (startDirectory == null) { throw new NullPointerException(
	 ...
	 *             if an I/O Error occurs
	 */
	private void walk(final File directory, final int depth,
			final Collection<T> results) throws IOException
	{
		...
		if (this.handleDirectory(directory, depth, results))
		{
			...
			{
				this.checkIfCancelled(directory, depth, results);
				File[] childFiles = this.filter == null ? directory.listFiles()
						: directory.listFiles(this.filter);
				childFiles = this.filterDirectoryContents(directory, depth,
						childFiles);
				if (childFiles == null)
				{
				...
				else
				{
					for (final File childFile : childFiles)
					{
						...
						{
							this.checkIfCancelled(childFile, childDepth,
									results);
