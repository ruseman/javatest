...
public class ByteArrayOutputStream extends OutputStream
{
	...
	public synchronized byte[] toByteArray()
	{
		...
		if (remaining == 0) { return ByteArrayOutputStream.EMPTY_BYTE_ARRAY; }
	...
	public void write(final byte[] b, final int off, final int len)
	{
		...
		synchronized (this)
		{
			...
			while (remaining > 0)
			{
				final int part = Math.min(remaining, this.currentBuffer.length
						- inBufferPos);
				System.arraycopy(b, (off + len) - remaining,
						this.currentBuffer, inBufferPos, part);
				remaining -= part;
				if (remaining > 0)
				{
	...
	private InputStream toBufferedInputStream()
	{
		...
		final List<ByteArrayInputStream> list = new ArrayList<ByteArrayInputStream>(
				this.buffers.size());
		for (final byte[] buf : this.buffers)
		{
			final int c = Math.min(buf.length, remaining);
