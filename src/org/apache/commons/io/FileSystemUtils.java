...
public class FileSystemUtils
{
	...
	{
		return FileSystemUtils.INSTANCE.freeSpaceOS(path, FileSystemUtils.OS,
				false, -1);
	}
	 ...
	 *             if an error occurs when finding the free space
	 */
	long freeSpaceOS(final String path, final int os, final boolean kb,
			final long timeout) throws IOException
	{
		...
		{
			case WINDOWS:
				return kb ? this.freeSpaceWindows(path, timeout)
						/ FileUtils.ONE_KB : this.freeSpaceWindows(path,
						timeout);
			case UNIX:
	 ...
	 *             if an error occurs
	 */
	long freeSpaceUnix(final String path, final boolean kb,
			final boolean posix, final long timeout) throws IOException
	{
		...
		final String[] cmdAttribs =
				...
				{
						FileSystemUtils.DF, flags, path
				} : new String[]
				{
						FileSystemUtils.DF, path
				};
...
// overflow)
		...
		if (lines.size() < 2)
		{
			...
			throw new IOException(
					"Command line '" + FileSystemUtils.DF
		...
		if (tok.countTokens() < 4)
		{
			...
			{
				final String line3 = lines.get(2); // the line may be interested
// in
	...
	long freeSpaceWindows(String path, final long timeout) throws IOException
	{
		...
		final List<String> lines = this.performCommand(cmdAttribs,
		...
		for (int i = lines.size() - 1; i >= 0; i--)
		{
			...
			if (line.length() > 0) { return this.parseDir(line, path); }
		}
		// all lines are blank
		throw new IOException(
	 ...
	 *             if an error occurs
	 */
	long parseBytes(final String freeSpace, final String path)
			throws IOException
	{
		try
		{
			final long bytes = Long.parseLong(freeSpace);
			if (bytes < 0) { throw new IOException(
					"Command line '" + FileSystemUtils.DF
	...
	long parseDir(final String line, final String path) throws IOException
	{
		...
		final StringBuilder buf = new StringBuilder(line.substring(bytesStart,
				bytesEnd));
	 ...
	 *             if an error occurs
	 */
	List<String> performCommand(final String[] cmdAttribs, final int max,
			final long timeout) throws IOException
	{
